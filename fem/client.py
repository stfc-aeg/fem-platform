...
class FemClient(object):
    ...
    def recv(self):
        ...
        if not data:
            raise FemClientError(
                "FEM has closed socket connection", FemClientError.ERRNO_SOCK_CLOSED
            )
    ...
    def acquireSend(self, theCmd, theMode=None, theBufSize=None, 
                    theBufCount=None, theNumAcqs=None, theCoalesce=None):
        ...
        self.send(theCmd=cmd, theBus=bus, theWidth=width, theState=state, 
                  theAddr=addr, thePayload=payload)
        response = self.recv()
        if response.state & FemTransaction.STATE_NO_ACKNOWLEDGE:
            (errorNo, errorStr) = response.decodeErrorResponse()
